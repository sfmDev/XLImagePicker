...
class XLImageBrowerView: UIScrollView, UIScrollViewDelegate {
        ...
        didSet {
            ...
            if imageSize.width < self.frame.width || imageSize.height < self.frame.height {
                if imageSize.width > imageSize.height {
                    ...
                    imageView.frame = CGRect(origin: CGPointZero, size: CGSize(width: self.frame.width, height: imageSize.height * ratio))
                } else {
                    // MARK: - width <= height
                    let ratio = self.frame.height / imageSize.height
                    imageView.frame = CGRect(origin: CGPointZero, size: CGSize(width: imageSize.width * ratio, height: self.frame.size.height))
                }
